...
<div class="fv-row mb-10 col-{{$width ?? 4}}" id='div-{{$wirePrefix}}-{{$field}}'>    
    <label class="form-label">@lang($label)</label>
        ...
        <x-crud::form.number field="{{$field . "_eligibility"}}" :wirePrefix="$wirePrefix" inputClass="form-control-solid" />
        <x-crud::form.number field="{{$field . "_score"}}" :wirePrefix="$wirePrefix" inputClass="form-control-solid" />
