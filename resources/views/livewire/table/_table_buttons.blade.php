<div class="btn-group" role="group" aria-label="group">
    ...
    @foreach($pageInfo['table']['buttons'] ?? [] as $option)
        @php($option = (object)$option)
            ...
            <!--begin::Menu item-->
                <a href="{{ $item->renderTags($option->url ?? '') }}" class="btn btn-sm btn-light btn-active-primary" target="{{ $option->target ?? '_self' }}" @if(!empty($option->action))wire:click.prevent="{{$option->action}}('{{$item->getKey()}}')" @if(!empty($option->confirm))wire:confirm="{{$option->confirm}}"@endif @endif wire:navigat>
