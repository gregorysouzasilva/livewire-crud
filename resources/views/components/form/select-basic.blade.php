...
<div class="form-floating">
    ...
    @endphp
        ...
        <select class="form-select {{$inputClass ?? ''}}"  id="{{$id}}" @if(!empty($disabled) || !empty($readOnly))disabled @endif @if(!empty($multiple)) multiple @endif wire:model{{ ($defer ?? false) ? '' : '.blur' }}="{{$wirePrefix}}{{$field ?? ''}}">
            <option></option>
