@if(($service->serviceType->histories[$serviceTypeHistory] ?? false) && (!in_array($contact->getCurrentStateRelation($stateRelation)->status, ['dismissed']) || hasRole('consultant')))
    <a href="{{route('clients.contacts.' . $routeName, ['client_uuid' => $client->uuid, 'contact_uuid' => $contact->uuid])}}" class="btn btn-light p-2 position-relative m-2" style="min-width: 50px;" wire:navigate>
