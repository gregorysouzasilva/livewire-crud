        ...
        <div class="menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-200px py-4 show"
            ...
            @if($item->evalTags($option->show) && 
                ...
                <div class="menu-item px-3">
                    <a href="{{ $item->renderTags($option->url ?? '') }}" class="menu-link px-3" target="{{ $option->target ?? '_self' }}" @if(!empty($option->action))wire:click.prevent="actionConfirm('{{$option->action}}','{{$item->getKey()}}', '{{$option->confirm ?? ''}}')"@endif wire:navigat>
